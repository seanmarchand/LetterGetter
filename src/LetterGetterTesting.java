...
public class LetterGetterTesting extends JPanel implements MouseListener, MouseMotionListener {
	File zero;
	...
	public LetterGetterTesting(String name) {
		super();
		index = 0;
		...
		btn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (pointEntered) {
					...
					analyze();
					char answer=' ';
					int indexFound = guess();
					try {
						BufferedReader br = new BufferedReader(new FileReader("list.txt"));
						try {
							String []testing=br.readLine().split(" ");
							answer=testing[indexFound].charAt(0);
					...
					if (JOptionPane.showConfirmDialog(null, "Is your character " + answer + "?", "Guess",
							JOptionPane.YES_NO_OPTION) == 1) {
						s = (String) JOptionPane.showInputDialog(fr, "Input the character you entered: ", "",
	...
	public void toFile() {
		q = newCharacter();
		try {
			File zero = new File(Integer.toString(q) + ".txt");
	...
	public int guess()  {
		...
		String line;
		int numAnalyzed[] = new int[leng];
		int[][] counters = new int[leng][500];
		int error = 30;
		...
		int first = 0;
		double[] average = new double[leng];
		for (int i = 0; i < leng; i++) {
			...
			// average[i]+=0.1;
			System.out.println(text[i] + " average is " + average[i]);
	...
	public int newCharacter() {
		...
		try {
			...
			try {
				...
				{
					BufferedWriter bw = new BufferedWriter(new FileWriter("list.txt", true));
		...
		for (int e = 0; e < temp.length; e++) {
			if (temp[e].charAt(0) != s.charAt(0) && e == temp.length - 1) {
				try {
					BufferedWriter bw = new BufferedWriter(new FileWriter("list.txt", true));
					bw.write(s);
					bw.write(" ");
			...
			} else if (temp[e].charAt(0) == s.charAt(0)) {
				return e;
			}
		}
		return -1;
	...
	public static void main(String[] args) {
		LetterGetterTesting mouse = new LetterGetterTesting("The Letter Getter");
	}
}